...
function OAuthHandlers(pluginType, request, resource, provider, sessionCallBack, clientOrigin) {
  ...
  switch (resource) {
    ...
    case "callback":
      switch (provider.algorithm) {
        case "oidc":
          return OIDCCallback(pluginType, request, provider, sessionCallBack).catch((error) => {
            console.error("OIDCCallback error", error);
            return Promise.resolve(Response.redirect(request.payload.config.serverURL));
          });
        case "oauth2":
